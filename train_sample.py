...
parser.add_argument(
        ...
        type=str,
        default='cnn_lstm',
        help='What model architecture to use among "simple_conv1D", "simple_conv2D" or "lstm".')
...
#TEST_FILE = os.path.join(FLAGS.train_dir,'test_'
...
with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1, FLAGS.num_epochs + 1):
            ...
            for training_step in range(1, int(train_set_size / FLAGS.batch_size) + 1):
                ...
                print('Step #%d: rate %f, loss %f, accuracy %.1f%%' %
                                                (training_step, FLAGS.learning_rate, train_loss, 
                                                 train_accuracy * 100))
                ...
                if (training_step % FLAGS.eval_step_interval) == 0:
                    ...
                    for i in range(0, valid_set_size, FLAGS.batch_size):
                        ...
                        # with the `merged` op.                        
                        validation_summary, validation_loss, validation_accuracy, conf_matrix, validation_predictions = sess.run(
                            [merged_summaries, loss, evaluation_step, confusion_matrix, predicted_indices])
                        ...
                        total_correct_labels.extend(valid_label_batch.eval())                    
                    
                    print("predictions",total_predictions)
                    print("correct labels", total_correct_labels)
                    """
                    print("prediction",sess.run(label_batch))
                    print("ground_truth",validation_predictions)
                    """
        ...
        # Testing loop
        print('TEST')
        test_set_size = get_dataset_shape(VALIDATION_FILE)
