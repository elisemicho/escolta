""" Preprocessing for audio files (need to be run only once)
from: https://github.com/swshon/dialectID_e2e """
...
FILENAME = '../../data/train_completesample.txt'
...
for i in range(TOTAL_JOB):
    TFRECORDS_NAME = '../../data/tfrecords/train_completesample.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
    TFRECORDS_NAME = '../../data/tfrecords/train_completesample_shuffle.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    
    
    
FILENAME = '../../data/dev_completesample.txt'
feat, utt_label, utt_shape, tffilename = feat_extract(FILENAME,FEAT_TYPE,N_FFT,HOP,VAD,CMVN,EXCLUDE_SHORT)
print tffilename

TOTAL_JOB = 1
for i in range(TOTAL_JOB):
    TFRECORDS_NAME = '../../data/tfrecords/dev_completesample.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
    feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
    TFRECORDS_NAME = '../../data/tfrecords/dev_completesample_shuffle.tfrecords'
    write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
...
# TOTAL_JOB = 1
# for i in range(TOTAL_JOB):
#     TFRECORDS_NAME = 'data/tfrecords/'+'mgb3_'+ tffilename +'_test.'+str(i)+'.tfrecords'
#     write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
#     feat, utt_label, utt_shape = do_shuffle(feat,utt_label,utt_shape)
#     TFRECORDS_NAME = 'data/tfrecords/'+'mgb3_'+ tffilename +'_test_shuffle.'+str(i)+'.tfrecords'
#     write_tfrecords(feat,utt_label,utt_shape,TFRECORDS_NAME)
